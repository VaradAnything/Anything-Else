...
import { ScrollHideConfig } from '../../directives/scroll-hide/catalogue-scroll-hide';
...
@Component
({
  selector: 'page-view-catalogue',
  templateUrl: 'view-catalogue.html',
})

export class ViewCataloguePage 
{
	public effect : any;
	...
	is_show_default_image: number = 1;
	...
	{
		this.category_title = this.navParams.get('category_title');
		this.cat_id = this.navParams.get('cat_id');
		this.sub_cat_id = this.navParams.get('sub_cat_id');
		this.sub_sub_cat_id = this.navParams.get('sub_sub_cat_id');
	}
    ...
    {
      	this.cssClass1 = "animated " + effect;
    }
    ...
    goToSearch()
  	...
  	getCatalogue(filter_expression,source_type)
  	{
	    ...
	    this.price = this.price_min_value + '-' + this.price_max_value;
	    this.moduleService.view_catalogue_data(this.cat_id,this.sub_cat_id,this.sub_sub_cat_id,filter_expression,this.q,this.sort_by,this.page,this.price,mobile_access_token).then((db_result: any) => 
	    {
	      	this.db_result = db_result;
	      	if(db_result.status_code == 200)
	      	{ 
  	...
  	addToCart(index,temp_photo1_size_medium,product_min_qty,product_max_qty)
  	{
	    ...
	    this.moduleService.add_to_cart(product_seo_url,product_variant_seo_url,product_min_qty,mobile_access_token).then((db_result: any) => 
	    {
	      	...
	      	{ 
	      		if(JSON.parse(localStorage.getItem('login_type')) == 0)
			  	{
			  		if(db_result.total_available_quantity > product_min_qty)
			  		{
				  		...
				  		if(without_login_cart_data_array == '' || without_login_cart_data_array == null)
			            {
				            ...
				            ({
				                ...
				                product_variant_quantity:  product_min_qty,
				                product_variant_one : '',
				                product_variant_two : '',
				            });
